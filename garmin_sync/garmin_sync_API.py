"""
Script to copy .fit files from the Garmin Connect API to the same location as
this script, and create a map of the GPS tracks contained therein overlaid on
...
GARMIN_TOKENS = HERE.joinpath("garmin_tokens")

LOGGER = logging.getLogger(__name__)


def download_fit_files():
    all_activities = garth.connectapi("/activitylist-service/activities/search/activities")

    outdir = HERE.joinpath("Activity")
    outdir.mkdir(exist_ok=True)

    LOGGER.info("Discovered %r activities from Connect API", len(all_activities))

    for act in all_activities:
        act_id = act['activityId']
        if outdir.joinpath(f"{act_id}_ACTIVITY.fit").exists():
            LOGGER.info("Activity %r has already been downloaded, skipping", act_id)
            continue

        LOGGER.info("Downloading Activity %r", act_id)
        archive_data = garth.download(f"/download-service/files/activity/{act_id}")

        zf = ZipFile(BytesIO(archive_data))
        for fn in (os.path.basename(zfn) for zfn in zf.namelist()):
            outfile = Path(outdir, fn)
            if outfile.exists():
                LOGGER.info("File already exists, skipping extraction: %r", str(outfile))
                continue
            else:
                LOGGER.info("Extracting FIT file: %r", str(outfile))
                zf.extract(fn, path=outdir)
...
def latlon_from_fit(infn: Path, fits_archive) -> np.ndarray:
    ...
    if "record_mesgs" not in messages:
        breakpoint()
        LOGGER.warning("'record_mesgs' key not defined, returning empty latlon array for %r", str(infn))
        return np.array([])
...
def osm_tile(x_tile, y_tile, zoom) -> PIL.Image.Image:
    pth = Path(HERE, "tiles", f"{x_tile}_{y_tile}_{zoom}.png")
    if pth.exists():
        LOGGER.debug("Using cached tile image")
        img = PIL.Image.open(pth)
    else:
        LOGGER.debug("Retrieving tile from OSM")
        ...
        # play nice, don't spam OSM
        time.sleep(1/OSM_RATE)
...
def plot(fits_files, osm: bool = True):
    ...
    for fn in fits_files:
        ...
        if infn.name in fits:
            LOGGER.info(f"Using cached lat/lon data for {fn!r}")
            latlon = fits[infn.name]
        else:
            LOGGER.info(f"Loading data for {fn!r} from FIT file")
            latlon = latlon_from_fit(infn, fits)
            fits[infn.name] = latlon

        if latlon.size == 0:
            LOGGER.warning("File has empty lat/lon data, skipping: %r", fn)
            continue
        ...
        if np.isinf([lonlim, latlim]).any():
            LOGGER.warning("Found infinite extents, skipping")
            continue
        ...
        plt.plot(lon, lat, 'b-', alpha=0.7, linewidth=2)
    ...
    if osm:
        ...
        assert (x1_tile - x0_tile) * (y1_tile - y0_tile) < 250, "That's too many tiles!"
    ...
    plt.savefig(outfn)
    LOGGER.info(f"GPS track plotted to {outfn!r}")
...
if __name__ == "__main__":
    if GARMIN_TOKENS.exists():
        ...
        LOGGER.warning("No existing Garmin Connect API token, performing authentication")
        username = input("Garmin Connect email: ")
        garth.login(username, getpass())
    ...
    # TODO: download fit files instead
    download_fit_files()
